...
Double-click **Write Blog** on the Desktop or in `~/Applications`. You can drag the app from Applications to your Dock. It starts the local server in the background if needed, reuses it if it is already running, and opens the admin page in your default browser. This also works after restarting your Mac. No Terminal window needs to stay open.
...
1. Open **Write Blog** and sign in with your existing administrator password. On a new installation, choose a password on the first local visit.
...
Public posts appear immediately on the local site. **Publish → Public** also builds the public pages and pushes them to GitHub Pages; the live site updates after GitHub finishes its deployment, usually within a few minutes. If that step fails, the post stays safely on this Mac and a **Retry** button appears. Pending publication survives a server restart. Private posts require your local admin login and remain on this Mac. Published posts cannot be changed or relabeled, in either the editor or the API.
...
Then open **Write Blog**. Alternatively, run `npm --prefix blog run open`, or keep `npm --prefix blog start` running in a Terminal window. Open `http://127.0.0.1:8080/` for the website or `http://127.0.0.1:8080/admin` to sign in. The homepage, blog, login, and editor use one server bound to this computer. The server continues running after the browser closes and stops when the Mac shuts down. Sessions end when the server restarts; posts and your password are preserved.
