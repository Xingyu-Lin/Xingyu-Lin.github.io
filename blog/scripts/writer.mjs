...
const directory = process.env.BLOG_LIBRARY_DIR || path.join(os.homedir(), '.local', 'share', 'xingyu-blog', createHash('sha256').update(repoRoot).digest('hex').slice(0, 12));
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (route.startsWith('/api/')) {
      ...
      if (!['/api/save', '/api/publish', '/api/archive', '/api/restore'].includes(route)) return json(res, 404, { error: 'Not found.' });
      if (writing) return json(res, 409, { error: 'Another save is running. Try again.' });
      writing = true;
      try {
        const input = await body(req);
        if (route === '/api/archive') return json(res, 200, { post: await store.archive(input) });
        if (route === '/api/restore') return json(res, 200, { post: await store.restore(input) });
        ...
        return json(res, 200, { post, url: visibility === 'public' ? `/blog/${post.slug}.html` : visibility === 'private' ? `/private/${post.slug}` : null });
